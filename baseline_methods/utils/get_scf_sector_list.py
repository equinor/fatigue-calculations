...
def get_scf_sector_list(geometry, compass_sectors):
    """
    ...
    Args:
        ...
        compass_sectors (list): of angles in degrees IN COMPASS FRAME, evenly distributed across the geometry 

    Returns:
        list, list: adjusted list of angles in degrees, a list of the scf factors in each of these adjusted angles
    """
    
    adjusted_angles = np.array(compass_sectors) # Initialize with all angles
    
    scf_per_point = np.ones_like(compass_sectors) * geometry['scf'] # If omni -> all angles have same SCF
    
    scf_sectors = np.array([geometry['orientation']])
    scf_sectors = scf_sectors[np.isfinite(scf_sectors)]
    
    if not scf_sectors.size == 0: # The compass_sectors are not omnidirectionally affected by stress
        # specific_angles = list(map(float, map(str.strip, geometry['scf_sector'].split(',')))) # Split by , remove spaces and convert 2 float 
        
        scf_per_point = np.ones_like(compass_sectors) # Initialize with scf = 1 for all angles
