...
def find_utils_subseabed_and_scale_lookup_table(info_path, lookup_path, worst_rule_path, member_geo_path, logger):
    ...
    worst_dd_tot = df_info.iloc[[0]]
    worst_dd_tot_elevation = worst_dd_tot['elevation'].iloc[0]
    worst_dd_tot_util = worst_dd_tot['Dd_tot'].iloc[0]
...
if __name__ == '__main__':
    ...
    lookup_table_paths = [os.path.join(path, name) for path, subdirs, files in os.walk(turbine_output_dir) for name in files if 'lookup_table.xlsx' in name]
    worst_util_rule_paths = [os.path.join(path, name) for path, subdirs, files in os.walk(turbine_output_dir) for name in files if 'worst_elevation' in name]
